...
@Component({
  selector: 'past-infra-grid',
  templateUrl: './past-infra-grid.component.html',
  styleUrls: ['./past-infra-grid.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class PastInfraGridComponent extends VrGridComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  @Output() totalStone: EventEmitter<any> = new EventEmitter<any>();
  @Output() colorLegendStone: EventEmitter<any> = new EventEmitter<any>();
  ...
  public dataToDisplayForSelection: any;
  ...
  // constructor(
  //   private stoneDetailService: StoneDetailsService,
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      this.totalStone.emit({
        totalStone: this.tableData.length, notAval: this.tableData.filter(id => id.stone_state === 6).length, seenStone: this.tableData.filter(id => id.stones_viewed > 0).length,
        onTableStone: this.tableData.filter(id => id.stones_on_table > 0).length, pendingStone: this.tableData.filter(id => id.viewRequestStatus === 1).length
      });
    }, 3000);
    ...
    setTimeout(() => {
      const elementName = 'grid' + this.gridName + '_scroll';
      const me = this;
      $('#' + elementName).on('scroll', function (evt) {
        const allGridArray = ['upcomingGrid', 'pastGrid', 'todayGrid', 'headerGrid'];
        const newScrolledGrid = _.without(allGridArray, me.gridName);
        newScrolledGrid.forEach(element => {
          const scrollInElement = 'grid' + element + '_scroll';
          if (document.getElementById(scrollInElement)) {
            document.getElementById(scrollInElement).scrollLeft = document.getElementById(elementName).scrollLeft;
          }
        });
      });
    }, 1000);
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.stonesActedOn && changes.stonesActedOn.firstChange === false) {
      ...
      if (this.gridInitialised === true) {
        ...
        if (changes.stonesActedOn.currentValue.source === 'filterStone') {
          setTimeout(() => {
            const tempData = this.transformDataForGrid(changes.stonesActedOn.currentValue.data);

            $('#gridpastGrid').igGrid('dataSourceObject', tempData).igGrid('dataBind');
      ...
      if (changes.selectedStonesArray.previousValue.length > changes.selectedStonesArray.currentValue.length) {
        ...
        for (let i = 0; i < changes.selectedStonesArray.previousValue.length; i++) {
          ...
          if (!changes.selectedStonesArray.currentValue.includes(changes.selectedStonesArray.previousValue[i])) {
            ...
            if (this.gridName === 'upcomingGrid') {
              this.currentSelectedRowsOfUpcoming = this.currentSelectedRowsOfUpcoming.filter(elm => { return elm.rowKey !== changes.selectedStonesArray.previousValue[i] });
              this.appStore.store('upcomingCurrentSelect', this.currentSelectedRowsOfUpcoming);
            }
            if (this.gridName === 'todayGrid') {
              this.currentSelectedRowsOfToday = this.currentSelectedRowsOfToday.filter(elm => { return elm.rowKey !== changes.selectedStonesArray.previousValue[i] });
              this.appStore.store('todayCurrentSelect', this.currentSelectedRowsOfToday);
            }
            if (this.gridName === 'pastGrid') {
              this.currentSelectedRowsOfPast = this.currentSelectedRowsOfPast.filter(elm => { return elm.rowKey !== changes.selectedStonesArray.previousValue[i] });
    ...
    if (changes.colorLegendFilter && changes.colorLegendFilter.firstChange === false && this.displayGridLoadingOverlay === false) {
      ...
      if (changes.colorLegendFilter.currentValue.length === 0) {
        ...
        $('#grid' + this.gridName).igGrid('dataSourceObject', tempData).igGrid('dataBind');
        this.displayGridLoadingOverlay = false;
        this.colorLegendStone.emit({ id: 'past', data: tempData });
    ...
    if (changes.submitStone && changes.submitStone.currentValue) {
      var gridArray = [];
      let allPastStoneData = this.appStore.getData('pastRequestArray');
      this.dataToDisplayForSelection = allPastStoneData['table'];
      gridArray = changes.submitStone.currentValue;
      if (gridArray.length > 0) {
        gridArray.forEach((elem, index) => {
          let temp = this.transformDataForGrid([elem]);
          if (this.dataToDisplayForSelection.filter(id => id._id === temp[0]._id).length > 0) {
  ...
  initTable() {
    ...
    $('#grid' + this.gridName).igGrid({
      ...
      features: [
        ...
        },
        {
          ...
          rowSelectorClicked: (evt, ui) => {
            let alreadySelectedpast = this.currentSelectedRowsOfPast.findIndex((elm, index) => {

              return elm.rowKey === ui.rowKey;

            });

            if (alreadySelectedpast > -1) {

            } else {

              this.currentSelectedRowsOfPast.push({ rowKey: ui.rowKey, rowIndex: ui.rowIndex });
              this.singleRowSelected.emit({ stoneId: ui.rowKey, added: true });

            }

            this.currentSelectedRowsOfPast.forEach(elm => {

              $('#grid' + this.gridName).igGridSelection('selectRowById', elm.rowKey);

            });

            alreadySelectedpast = null;
            // for upcoming 
            let alreadySelectedOfUpcoming = this.currentSelectedRowsOfUpcoming.findIndex((elm, index) => {

              return elm.rowKey === ui.rowKey;

            });

            if (alreadySelectedOfUpcoming > -1) {

            } else {

              this.currentSelectedRowsOfUpcoming.push({ rowKey: ui.rowKey, rowIndex: ui.rowIndex });
              this.singleRowSelected.emit({ stoneId: ui.rowKey, added: true });

            }

            this.currentSelectedRowsOfUpcoming.forEach(elm => {

              $('#grid' + this.gridName).igGridSelection('selectRowById', elm.rowKey);

            });

            alreadySelectedOfUpcoming = null;
            let alreadySelectedOfToday = this.currentSelectedRowsOfToday.findIndex((elm, index) => {

              return elm.rowKey === ui.rowKey;

            });

            if (alreadySelectedOfToday > -1) {

            } else {

              this.currentSelectedRowsOfToday.push({ rowKey: ui.rowKey, rowIndex: ui.rowIndex });
              this.singleRowSelected.emit({ stoneId: ui.rowKey, added: true });

            }

            this.currentSelectedRowsOfToday.forEach(elm => {

              $('#grid' + this.gridName).igGridSelection('selectRowById', elm.rowKey);

            });

            alreadySelectedOfToday = null;
      ...
      cellClick: (evt, ui) => {
        ...
        if (this.currentSelectedRowsOfPast) {
          let alreadySelected = this.currentSelectedRowsOfPast.findIndex(elm => {
          ...
          if (alreadySelected < 0) {
            ...
            $('#gridpastGrid').igGridSelection('deselectRowById', ui.rowKey);

          }
          alreadySelected = null;

        }
        if (this.currentSelectedRowsOfToday) {
          let alreadySelected = this.currentSelectedRowsOfToday.findIndex(elm => {
          ...
          if (alreadySelected < 0) {
            ...
            // Deselect row since we don't want to select row by clicking the cells.
            $('#gridtodayGrid').igGridSelection('deselectRowById', ui.rowKey);

          }
          alreadySelected = null;

        }
        if (this.currentSelectedRowsOfUpcoming) {
          let alreadySelected = this.currentSelectedRowsOfUpcoming.findIndex(elm => {
          ...
          if (alreadySelected < 0) {
            ...
            // Deselect row since we don't want to select row by clicking the cells.
            $('#gridupcomingGrid').igGridSelection('deselectRowById', ui.rowKey);
        ...
        this.currentSelectedRowsOfPast = this.appStore.getData('pastCurrentSelect');
        if (this.currentSelectedRowsOfPast) {
          this.currentSelectedRowsOfPast.map(elm => {

            $('#gridpastGrid').igGridSelection('selectRowById', elm.rowKey);

          });
        } else {
          this.currentSelectedRowsOfPast = [];
        }
        this.currentSelectedRowsOfToday = this.appStore.getData('todayCurrentSelect');
        if (this.currentSelectedRowsOfToday) {
          this.currentSelectedRowsOfToday.map(elm => {

            $('#gridtodayGrid').igGridSelection('selectRowById', elm.rowKey);
        ...
        this.currentSelectedRowsOfUpcoming = this.appStore.getData('upcomingCurrentSelect');
        if (this.currentSelectedRowsOfUpcoming) {
          this.currentSelectedRowsOfUpcoming.map(elm => {

            $('#gridupcomingGrid').igGridSelection('selectRowById', elm.rowKey);
      ...
      return stoneArray.map((elm) => {
        ...
        return {
          'multimedia': {
            ...
            stoneStatus: elm.stone_state === 6 ? 'Not Available' : elm.viewRequestStatus === 1 ? 'Pending' : elm.viewRequestStatus === 2 ? 'SEEN' : elm.stones_on_table.length > 0 ? 'On Table' : ''
          },
          'viewDate': elm.view_date_time ? this.utilSvc.returnHTMLNeededDateTimeFormat(elm.view_date_time, 'dateWithMonth') + ',' + this.utilSvc.returnHTMLNeededDateTimeFormat(elm.view_date_time, 'year') : '',
          'viewTime': elm.view_date_time ? this.utilSvc.returnHTMLNeededDateTimeFormat(elm.view_date_time, 'timeAMPM') : '',
          'viewNote': elm.view_request_note.length > 0 ? elm.view_request_note[elm.view_request_note.length - 1].comment : '',
  ...
  updateIconDataForStones(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.stoneDetailService
        .storeStoneAdditionalInfo(this.resultObject.table || this.resultObject.diamondTable)
        .subscribe(res => {

          const temp = this.stoneDetailService.addStoneAdditionalInfo(this.tableData, res);
    ...
    if (ui.isHeader) {
      ...
      if (ui.state === 'on') {
        ...
        if (this.resultObject.filteredStone && this.resultObject.filteredStone.length === 0) {
          ...
          if (this.gridName === 'upcomingGrid') {
            this.currentSelectedRowsOfUpcoming = this.currentSelectedRowsOfUpcoming.filter(elm => {
              return elm.rowKey !== ui.rowKey;
            });
            this.appStore.store('upcomingCurrentSelect', this.currentSelectedRowsOfUpcoming);
          }
          if (this.gridName === 'todayGrid') {
            this.currentSelectedRowsOfToday = this.currentSelectedRowsOfToday.filter(elm => {
              return elm.rowKey !== ui.rowKey;
            });
            this.appStore.store('todayCurrentSelect', this.currentSelectedRowsOfToday);
          }
          if (this.gridName === 'pastGrid') {
            this.currentSelectedRowsOfPast = this.currentSelectedRowsOfPast.filter(elm => {
              return elm.rowKey !== ui.rowKey;
            });
            this.appStore.store('pastCurrentSelect', this.currentSelectedRowsOfPast);
          }
  ...
  public afterGridRendered() {
    ...
    setTimeout(() => {
      ...
      if (this.dataToDisplay.length > 35) {
        ...
        $('#gridpastGrid').igGrid('dataSourceObject', this.dataToDisplay).igGrid('dataBind');
