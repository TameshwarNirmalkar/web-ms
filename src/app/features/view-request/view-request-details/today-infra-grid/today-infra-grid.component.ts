...
@Component({
  selector: 'today-infra-grid',
  templateUrl: './today-infra-grid.component.html',
  styleUrls: ['./today-infra-grid.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class TodayInfraGridComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  //Store Data for selected Stone Button Click Event 
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      $('#gridtodayGrid_scroll').on('scroll', function () {
        document.getElementById('gridupcomingGrid_scroll').scrollLeft = document.getElementById('gridtodayGrid_scroll').scrollLeft;
        document.getElementById('gridheaderGrid_scroll').scrollLeft = document.getElementById('gridtodayGrid_scroll').scrollLeft;
        document.getElementById('gridpastGrid_scroll').scrollLeft = document.getElementById('gridtodayGrid_scroll').scrollLeft;
  ...
  ngOnChanges(changes: SimpleChanges) {
      ...
      if (this.gridInitialised === true) {
        ...
        if (changes.stonesActedOn.currentValue.source === 'filterStone') {
          setTimeout(() => {
            ...
            $('#gridtodayGrid').igGrid('dataSourceObject', tempData).igGrid('dataBind');
    ...
    if (changes.colorLegendFilter && changes.colorLegendFilter.firstChange === false && this.displayGridLoadingOverlay === false) {
     ...
     if (this.tableData.length > 0) {
      ...
      if (changes.colorLegendFilter.currentValue.length === 0) {
        ...
        $('#grid' + this.gridName).igGrid('dataSourceObject', tempData).igGrid('dataBind');
        this.displayGridLoadingOverlay = false;
        this.colorLegendStone.emit({id: 'today' , data : tempData});
    ...
    if (changes.submitStone && changes.submitStone.currentValue) {
      var gridArray = [];
      let allPastStoneData = this.appStore.getData('stoneRequestedArray');
  ...
  initTable() {
    ...
    $('#grid' + this.gridName).igGrid({
      ...
      cellClick: (evt, ui) => {
        ...
        this.currentSelectedRows = this.appStore.getData('todayCurrentSelect');
        if (this.currentSelectedRows) {
  ...
  searchValues(searchString) {
    ...
    this.displayGridLoadingOverlay = true;
    // const currentData = $('#gridtodayGrid').data('igGrid').dataSource.dataView();
    let allTodayeData = this.appStore.getData('stoneRequestedArray');
    this.dataToDisplay = this.transformDataForGrid(allTodayeData.table);
    let tempArray = [];
  ...
  public handleCheckboxStateChange(evt: any, ui: any) {
    if (ui.isHeader) {
      ...
      if (ui.state === 'on') {
        ...
        this.tableData =  this.appStore.getData('stoneRequestedArray');
        if (this.tableData.selectedStones.length > 0) {
          const currentData = $('#gridtodayGrid').data('igGrid').dataSource.dataView();
          ...
          if (currentData.length > 0) {
            ...
            this.appStore.store('todayCurrentSelect', this.currentSelectedRows);
