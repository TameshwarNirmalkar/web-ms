...
import { StoneDetailsService, StoneMultimediaDetailDirective } from '@srk/shared';
...
@Component({
  selector: 'upcoming-infra-grid',
  templateUrl: './upcoming-infra-grid.component.html',
  styleUrls: ['./upcoming-infra-grid.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class UpcomingInfraGridComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      $('#gridupcomingGrid_scroll').on('scroll', function () {
        document.getElementById('gridpastGrid_scroll').scrollLeft = document.getElementById('gridupcomingGrid_scroll').scrollLeft;
        document.getElementById('gridheaderGrid_scroll').scrollLeft = document.getElementById('gridupcomingGrid_scroll').scrollLeft;
        document.getElementById('gridtodayGrid_scroll').scrollLeft = document.getElementById('gridupcomingGrid_scroll').scrollLeft;
  ...
  ngOnChanges(changes: SimpleChanges) {
      ...
      if (this.gridInitialised === true) {
        ...
        if (changes.stonesActedOn.currentValue.source === 'filterStone') {
          setTimeout(() => {
            ...
            $('#gridupcomingGrid').igGrid('dataSourceObject', tempData).igGrid('dataBind');
    ...
    if (changes.colorLegendFilter && changes.colorLegendFilter.firstChange === false && this.displayGridLoadingOverlay === false) {
      ...
      if (this.tableData.length > 0) {
        ...
        if (changes.colorLegendFilter.currentValue.length === 0) {
          ...
          $('#grid' + this.gridName).igGrid('dataSourceObject', tempData).igGrid('dataBind');
          this.colorLegendStone.emit({id: 'upcoming' , data :tempData});
          this.displayGridLoadingOverlay = false;
    ...
    if (changes.updateIcon && changes.updateIcon.currentValue) {
      var gridArray = [];
      gridArray = changes.updateIcon.currentValue;
      if (gridArray.length > 0) {
        gridArray.forEach((elem, index) => {
          let temp = this.transformDataForGrid([elem]);
          $('#gridupcomingGrid').igGridUpdating('updateRow', temp[0]._id, temp[0]);
        });
      }
    }
    ...
    if (changes.submitStone && changes.submitStone.currentValue) {
      var gridArray = [];
      let allPastStoneData = this.appStore.getData('upcomingRequestArray');
  ...
  ngOnDestroy() {
    ...
    this.saveGridState()
      .then(saved => {
        if (saved !== null) {
        $('#gridupcomingGrid').igGrid('destroy');
        }
        ...
        const gridStateKey = 'gridupcomingGrid' + 'State';
  ...
  initTable() {
    ...
    $('#grid' + this.gridName).igGrid({
      ...
      cellClick: (evt, ui) => {
        ...
        this.currentSelectedRows = this.appStore.getData('upcomingCurrentSelect');
        if (this.currentSelectedRows) {
    ...
    return stoneArray.map((elm) => {
      ...
      return {
        'multimedia': {
          ...
          viewDate: elm.view_date_time ? moment(date).format("DD-MMM-YYYY") : '',
          viewTime: elm.view_date_time ? moment(date).format("hh:mm A") : '',
  ...
  generateTableColumns(): void {
    ...
    this.tableColumns.push({
      ...
      dataType: 'string',
      columnIndex: 1,
      template: '<lable>${multimedia.viewDate}</lable>'
    });
    ...
    this.tableColumns.push({
      ...
      dataType: 'string',
      columnIndex: 2,
      template: '<lable>${multimedia.viewTime}</lable>'
    });
    ...
    this.tableColumns.push({
      ...
      dataType: 'string',
      columnIndex: 3,
      template: '<lable>${multimedia.viewNote}</lable>'
    });
    ...
    this.tableColumns.push({
      ...
      dataType: 'string',
      columnIndex: 4,
      template: '<lable>${multimedia.stoneStatus}</lable>'
    });
  ...
  searchValues(searchString) {
    ...
    // const currentData = $('#gridupcomingGrid').data('igGrid').dataSource.dataView();
    let allUpcomingData = this.appStore.getData('upcomingRequestArray');
    if (allUpcomingData.table) {
      this.dataToDisplay = this.transformDataForGrid(allUpcomingData.table);
    }
    let tempArray = [];
    ...
    //     const patt = new RegExp(searchString, 'gim');

    //     if (patt.test(JSON.stringify(currentData[i]))) {
  ...
  public saveGridState(): Promise<any> {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      gridStateObject.sortingState = $('#gridupcomingGrid').igGridSorting('option', 'columnSettings');
      // gridStateObject.filteringExpressions = $('#gridgridupcomingGrid').data('igGrid').dataSource.settings.filtering.expressions;
    ...
    if (ui.isHeader) {
      ...
      if (ui.state === 'on') {
        ...
        if (currentData.length > 0) {
          ...
          this.appStore.store('upcomingCurrentSelect', this.currentSelectedRows);
