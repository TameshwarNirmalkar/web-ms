...
import { StoneDetailsService, StoneMultimediaDetailDirective, UtilService, AddNoteService } from '@srk/shared';
import { ApplicationAuditService, SessionStorageService, AuthService, UserProfileService, ApplicationStorageService } from '@srk/core';
...
@Component({
  selector: 'app-infra-grid',
  templateUrl: './infra-grid.component.html',
  styleUrls: ['./infra-grid.component.scss']
})

export class InfraGridComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  private leftScroll: any = 0;
  public verticalScroll: number = 0;
  private initGrid: Boolean = true;
  public gridInitialised: Boolean = false;
  public oldColumnIndex = [];
  public newColumnIndex = [];
  ...
  private gridFixingDirection: String = 'left';
  ...
  constructor(
    ...
    private stoneMultiMediaDetailDirective: StoneMultimediaDetailDirective,
  ...
  adjustFixedUnfixedRowWhenScrollingStops() {
    if (document.getElementById('grid' + this.gridName + '_displayContainer')
      && document.getElementById('grid' + this.gridName + '_fixedBodyContainer')) {
      document.getElementById('grid' + this.gridName + '_displayContainer').scrollTop =
        document.getElementById('grid' + this.gridName + '_fixedBodyContainer').scrollTop;
    }
  }

  ngOnChanges(changes: SimpleChanges) {
    console.log("Going in infragrid" + this.gridName, changes)
    ...
    if (changes.stonesActedOn && changes.stonesActedOn.firstChange === false &&
      changes.stonesActedOn.currentValue && changes.stonesActedOn.currentValue.source) {

      // To avoid refresh of while updating price in case of DAYP events
      const daypEvents = ['daypStoneStateUpdated', 'DAYPStoneOfferUpdated', 'daypPriceMidUpdated',
        'offerOnSelectedStonesUpdated'];
      const b2bEvents = ['bidToBuyPriceUpdated', 'bidToBuyAllStoneUpdate', 'b2bRequested', 'b2bSubmitted',
        'bidToBuyPSubmitedUpdated', 'b2bPriceMidUpdated', 'b2bPriceUpdated', 'b2bPriceInserted', 'b2bDeleted'];
      console.log("1---", daypEvents.indexOf(changes.stonesActedOn.currentValue.source), b2bEvents.indexOf(changes.stonesActedOn.currentValue.source))
      if (daypEvents.indexOf(changes.stonesActedOn.currentValue.source) === -1
        && b2bEvents.indexOf(changes.stonesActedOn.currentValue.source) === -1) {
        this.displayGridLoadingOverlay = true;
        ...
        if (this.gridInitialised === true) {
          ...
          console.log("Inside infragrid", changes.stonesActedOn)
          if (changes.stonesActedOn.currentValue.source === 'confirmedStones' || changes.stonesActedOn.currentValue.source === 'holdRequested') {
            ...
            if (changes.stonesActedOn.currentValue.stoneList.length > 50) {
              changes.stonesActedOn.currentValue.stoneList.forEach((elem, index) => {
                gridObj.dataSource.deleteRow(elem, true);
                ...
                if (index === (changes.stonesActedOn.currentValue.stoneList.length - 1)) {
                  ...
                  $('#grid' + this.gridName).igGrid('dataSourceObject', tempArray).igGrid('dataBind');
                  this.realignGrid();
                }
              });
            } else {
              changes.stonesActedOn.currentValue.stoneList.forEach((elem, index) => {
                if (grid.find('tr[data-id=' + elem + ']').length > 0) {
          ...
          } else if (changes.stonesActedOn.currentValue.source === 'noteAdded') {
            const grid = $('#grid' + this.gridName);
            const gridObj = grid.data('igGrid');
            let stoneList = [];
            if (changes.stonesActedOn.currentValue.stoneList) {
              stoneList = changes.stonesActedOn.currentValue.stoneList;
            } else {
              stoneList = _.pluck(this.tableData, '_id');
            }
            if (stoneList.length < 35) {
              tempArray.forEach((elm, index) => {
                if (stoneList.indexOf(elm._id) > -1) {
                  if (grid.find('tr[data-id=' + elm._id + ']').length > 0) {
                    grid.igGridUpdating('updateRow', elm._id, elm);
                  } else {
                    gridObj.dataSource.updateRow(elm._id, elm);
                  }
                }
                if (tempArray.length - 1 === index) {
                  setTimeout(() => {
                    // const height = this.getGridHeight(); This is causing issue in search page
                    // $('#grid' + this.gridName).igGrid('option', 'height', height);
                    this.displayGridLoadingOverlay = false;
                  }, 500);
                }
              });
    ...
    if (changes.selectedStoneTableIsVisible && changes.selectedStoneTableIsVisible.firstChange === false) {
      ...
      if (this.selectedStoneTableIsVisible === false) {
        ...
        this.resultObject.selectedStoneButton.map(elm => {
    ...
    if (changes.selectedStonesArray && changes.selectedStonesArray.firstChange === false) {
      console.log("Selected stone array", changes.selectedStonesArray)
      if (changes.selectedStonesArray.previousValue.length > changes.selectedStonesArray.currentValue.length) {
        ...
        for (let i = 0; i < changes.selectedStonesArray.previousValue.length; i++) {
          ...
          if (!changes.selectedStonesArray.currentValue.includes(changes.selectedStonesArray.previousValue[i])) {
            ...
            this.currentSelectedRows = this.currentSelectedRows.filter(elm => { return elm.rowKey !== changes.selectedStonesArray.previousValue[i] });
    ...
    if (changes.visiblePacketIcon && changes.visiblePacketIcon.firstChange === false) {
      ...
      if (changes.visiblePacketIcon.currentValue === true) {
        ...
        this.dataToDisplay = this.transformDataForGrid(this.tableData);
    ...
    if (changes.packetIconDataForGrid && changes.packetIconDataForGrid.firstChange === false) {
      ...
      let temp = changes.packetIconDataForGrid.currentValue.split(',');
      const allStones = this.stoneDetailService.createStoneIdList(this.tableData)
      const matchingStones = _.intersection(allStones, temp);
      const notMatchedStones = _.difference(allStones, temp);
      for (let i = 0; i < matchingStones.length; i++) {
        $('#grid' + this.gridName)
          .igGrid('cellById', matchingStones[i], 'addtopacket').children().removeClass('icon-packet-add').addClass('icon-packet-added text-green');
      }

      for (let i = 0; i < notMatchedStones.length; i++) {
        $('#grid' + this.gridName)
          .igGrid('cellById', notMatchedStones[i], 'addtopacket').children().removeClass('icon-packet-added text-green').addClass('icon-packet-add');
      }
    ...
    if (changes.colorLegendFilter && changes.colorLegendFilter.firstChange === false && this.displayGridLoadingOverlay === false) {
      ...
      let gridData = this.transformDataForGrid(this.tableData);
      let tempData: any[] = [];
      ...
      if (changes.colorLegendFilter.currentValue.length === 0) {
        ...
        tempData = this.transformDataForGrid(this.tableData);
        $('#grid' + this.gridName).igGrid('dataSourceObject', tempData).igGrid('dataBind');
        this.displayGridLoadingOverlay = false;
  ...
  adjustScrollTopForFixedAndUnfixedDivs() {
    setTimeout(() => {
      if (document.getElementById('grid' + this.gridName + '_displayContainer')
        && document.getElementById('grid' + this.gridName + '_fixedBodyContainer')) {
        document.getElementById('grid' + this.gridName + '_displayContainer').scrollTop =
  ...
  initTable() {
    $('#grid' + this.gridName).igGrid({
      ...
      width: '100%',
      height: this.calculatedGridHeight,
      avgRowHeight: 35,
      ...
      features: [
        ...
        },
        {
          ...
          columnSettings: [
            ...
            const fixedCols = ['addtopacket', 'multimedia', '_id'];
            if (args && fixedCols.indexOf(args.columnIdentifier) === -1) {
              $('#grid' + this.gridName).igGridColumnFixing('unfixColumn', args.columnIdentifier, false);
            }
            this.realignGrid();
        ...
        },
        {
          ...
          columnMoved: (evt, args) => {
            ...
            args.newIndex = movedColumnIndex;
            this.oldColumnIndex.push(args.oldIndex);
            this.newColumnIndex.push(movedColumnIndex);


            this.columnMovedArray.push(args);
            this.appStore.store('oldIndex', this.oldColumnIndex);
            this.appStore.store('newIindex', this.newColumnIndex);
        ...
        },
        {
          ...
          columnSettings: [
            ...
            { 'columnKey': 'btbEndDate', 'allowTooltips': false },
      ...
      cellClick: (evt, ui) => { 
        ...
        if (ui.colKey === 'addtopacket') {
          ...
          let temp = this.tableData.find(elm => {
        ...
        if (ui.colKey === 'certificate') {
          ...
          const temp = this.tableData.find(elm => {
    ...
    return stoneArray.map((elm) => {
      ...
      return {
        'multimedia': {
          ...
          btobstate: elm.b2b_state === 1 ? (elm.isBtbUpdated === 1 ?
            'icon-b2b text-red' : (elm.isBtbUpdated === 2 ? 'icon-b2b text-green' : 'icon-b2b')) : 'null',
  ...
  public realignGrid(): void {
    let headerWidth = $('#grid' + this.gridName + '_headers thead').width();

    $('#grid' + this.gridName).css({
    ...
    if (!this.isSelectedPanel) {
      const observables = [
        this.stoneDetailService
          .storeStoneAdditionalInfoForGrid(this.resultObject.table || this.resultObject.diamondTable),
        this.notesService.getCommentListforStoneIdsForGrid(this.resultObject.table || this.resultObject.diamondTable)
      ];
      return new Promise((resolve, reject) => {
        Observable.forkJoin(observables).subscribe(
          response => {
            if (!response[0].error_status) {
              temp = this.stoneDetailService.addStoneAdditionalInfo(this.tableData, response[0].data);
            }
            if (response[1].data) {
              temp = this.notesService.addStoneNotesInfo(temp, response[1].data);
            } else {
              temp = this.notesService.removeStoneNotesInfo(temp);
            }
            resolve(temp);
          },
        );
  ...
  searchValues(searchString) {
    ...
    let tempArray = [];
    // if (searchString === '') {
    //   tempArray = this.dataToDisplay;
    // }
    //  else {
    //   for (let i = 0; i < currentData.length; i++) {
    //     const patt = new RegExp(searchString, 'gi');
    //     const isTrue = patt.test(JSON.stringify(currentData[i]));
    //     if (isTrue) {
    //       tempArray.push(this.dataToDisplay.find(elm => { return elm._id === currentData[i]._id; }));
    //     }
    //   }
    // }
    for (let i = 0; i < this.dataToDisplay.length; i++) {
      const patt = new RegExp(searchString, 'gi');
      if (patt.test(JSON.stringify(this.dataToDisplay[i]))) {
        tempArray.push(this.dataToDisplay[i]);
      }
    }
    ...
    setTimeout(() => {
      ...
      // $('#grid' + this.gridName + '_hscroller').animate({ scrollLeft: this.leftScroll }, 0);

      // This is added to solved issue related to search and no stones found while data is manipulated from selected panel 
      if (document.getElementById('grid' + this.gridName + '_scrollContainer')
        ...
        document.getElementById('grid' + this.gridName + '_scrollContainer').scrollTop = Number(this.verticalScroll);
        $('#grid' + this.gridName).igGrid('commit');
        this.displayGridLoadingOverlay = false;
      }
      // Restore vertical scroll.
      // $('#grid' + this.gridName).igGrid('virtualScrollTo', this.verticalScroll.toString());
  ...
  applyContextMenuActionToGrid(action: { label: String, value: String, columnName: String }): void {
    ...
    switch (action.value) {
      ...
      case 'sortAscending':
        ...
        $('#grid' + this.gridName).igGridSorting('sortColumn', action.columnName, 'ascending');

        break;

      case 'sortDescending':

        $('#grid' + this.gridName).igGridSorting('sortColumn', action.columnName, 'descending');

        break;

      case 'clearSorting':

        $('#grid' + this.gridName).igGridSorting('unsortColumn', action.columnName);

        let column = $('#grid' + this.gridName).igGrid('option', 'columns').find(elm => { return elm.key === action.columnName; });

        if (column && (column.width === (column.originalWidth + 16))) {

          $('#grid' + this.gridName).igGridResizing('resize', action.columnName, column.originalWidth);

        }

        column = null;

        const filteringExpressions = $('#grid' + this.gridName).data('igGrid').dataSource.settings.filtering.expressions;

        $('#grid' + this.gridName).igGridFiltering('filter', filteringExpressions, true);
      ...
      case 'hideColumn':
        ...
        $('#grid' + this.gridName).igGridHiding('hideColumn', action.columnName, () => {
      ...
      case 'moveLast':
        ...
        $('#grid' + this.gridName).igGrid('moveColumn', action.columnName, (this.tableColumns.length - 1), true, false);
      ...
      case 'moveFirst':
        ...
        $('#grid' + this.gridName).igGrid('moveColumn', action.columnName, (idColumnIndex + 1), true, false);
  ...
  public saveGridState(): Promise<any> {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      // gridStateObject.horizontalScroll = document.getElementById('grid' + this.gridName + '_hscroller') ? 
      // document.getElementById('grid' + this.gridName + '_hscroller').scrollLeft : 0;
      ...
      this.appStore.store('oldIndex', gridStateObject.oldIndex);
      this.appStore.store('newindex', gridStateObject.newIndex);
    ...
    this.sessionStorageService.getDataFromSessionStorage(gridStateKey)
      .then((gridState) => {
        ...
        if (gridState) {
          ...
          if (gridState.gridStateSwapping.length > 0) {
            if (this.tableColumns[0].key === "addtopacket") {

              for (let packetGridIndex = 0; packetGridIndex <= gridState.oldIndex.length - 1; packetGridIndex++) {
                setTimeout(() => {
                  $('#grid' + this.gridName).igGrid("moveColumn", gridState.oldIndex[packetGridIndex] + 3, gridState.newIndex[packetGridIndex]);
                  this.realignGrid();

                }, 500)
              }
            } else {
              for (let gridIndex = 0; gridIndex <= gridState.oldIndex.length - 1; gridIndex++) {
                setTimeout(() => {
                  $('#grid' + this.gridName).igGrid("moveColumn", gridState.oldIndex[gridIndex] + 2, gridState.newIndex[gridIndex]);
                  this.realignGrid();
                }, 500)
              }
        ...
        // Restore vertical scroll.
        setTimeout(() => {
          if (gridState && gridState.verticalScroll) {
            document.getElementById('grid' + this.gridName + '_scrollContainer').scrollTop = gridState.verticalScroll;
            // $('#grid' + this.gridName).igGrid('virtualScrollTo', val.toString() + 'px');
          }
        }, 1000);
  ...
  public handleCheckboxStateChange(evt: any, ui: any) {
    ...
    if (ui.isHeader) {
      ...
      setTimeout(() => {
        document.getElementById('grid' + this.gridName + '_scrollContainer').scrollTop = Number(this.currentVerticalScrollPosition);
  ...
  public afterGridRendered() {
    ...
    this.assignScrollerCheckForGrid();

  }

  assignScrollerCheckForGrid() {
    setTimeout(() => {
      const elementName = 'grid' + this.gridName + '_displayContainer';
      const me = this;
      let timer;
      document.getElementById(elementName).onscroll = function (evt) {
        clearTimeout(timer);
        timer = setTimeout(() => {
          me.adjustFixedUnfixedRowWhenScrollingStops();
        }, 500);
      };
    }, 1000);
  }
