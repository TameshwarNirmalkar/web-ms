...
gulp.task('compress', function () {
  ...
  return gulp
    .src([
      'dist/login-app/*.{js,css,png,jpg,gif,svg,eot,ttf,woff,woff2,txt}',
      'dist/login-app/!login-index.html'
    ], {base: './dist/login-app'})
    .pipe(gzip({
      append: false,
      threshold: 860,
    ...
    .pipe(gulp.dest('./dist/login-app/final/'));
...
gulp.task('copyAssets', function () {
  ...
  return gulp
    .src(['dist/login-app/assets/**', 'dist/login-app/login-index.html'], {base: './dist/login-app'})
    .pipe(gulp.dest('dist/'));

});

// gulp.task('default', ['compress', 'copyAssets']);
...
gulp.task('default', ['compress', 'copyAssets'], function () {
  ...
  let files = glob.sync('./dist/login-app/final/**');
    ...
    .then(response => {
      ...
      if (response.success) {
        ...
        for(let i = 0; i < response.data.result.entries.length; i++){
          ...
          let index = files.indexOf(response.data.result.entries[i].name.replace(prefix, "./dist/login-app/final/"));
        ...
        let uploads = files.map(file => {
          ...
          return azureService.uploadFileToAzureStorageForLogin(blobService, containerName, directoryName, file)
            .then( uploaded => {
              return uploaded;
            })
            .catch( failed => {
          ...
          .then( data => {
            ...
            if(failedFiles.length > 0 ){
              ...
              let retryUpload = failedFiles.map(retryFile => {
                ...
                return azureService.uploadFileToAzureStorageForLogin(blobService, containerName, directoryName, retryFile)
                  .then( uploaded => {
                    return uploaded;
                  })
                  .catch( failed => {
